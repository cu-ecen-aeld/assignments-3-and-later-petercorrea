...
// fork, update file permissions, open new sess and close standard file descriptors
void create_daemon()
{
    pid_t pid = fork(), sid;
    if (pid < 0)
    ...
    umask(0);
    if ((sid = setsid()) < 0) // isolate the child process from the parent's session and process group
        exit(EXIT_FAILURE);
    close(STDIN_FILENO);
    close(STDOUT_FILENO);
    close(STDERR_FILENO);
...
void process_client(int client_sock, struct sockaddr_in client_addr)
{
    ...
    {
        if (total_received > mallinfo().fordblks) // Check if buffer exceeds heap
        {
